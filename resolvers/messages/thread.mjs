    ...
    client.users.fetch(client.userLib.threads.get(msg.channel.id)).then((user) => {
        if (action === 'send') {
            user.send(opt).then((sendedMsg) => {
                ...
                client.userLib.tickets.get(user.id).messageLinks.set(msg.id, sendedMsg.id);
            })
        } else {
            client.users.fetch(client.userLib.threads.get(msg.channel.id)).then((user) => {
                user.dmChannel.messages.fetch(client.userLib.tickets.get(user.id).messageLinks.get(msg.id)).then(fetchedMessage => {
                    if (action === 'edited') {
